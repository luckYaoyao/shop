<template>
	<view class="product-con" :style="colorStyle">
					...
					<view class="wrapper">
						<view class="share acea-row row-between row-bottom">
							<view class="money font-color">
								...
								<image v-if="storeInfo.vip_price && storeInfo.vip_price > 0 && storeInfo.is_vip == 1"
		...
		<view class="footer acea-row row-between-wrapper">
			...
			<navigator open-type="reLaunch" class="animated item" :class="animated == true ? 'bounceIn' : ''"
			...
			<view>
				<view class="bnt acea-row" :class="storeInfo.is_virtual?'virbnt':''"
					v-if="attr.productSelect.stock <= 0">
					<form v-if="!storeInfo.is_virtual" @submit="joinCart" class="joinCart bnts"
						:class="storeInfo.is_virtual?'virbnt':''"><button class="joinCart bnts"
							form-type="submit">加入购物车</button></form>
					<form class="buy bnts bg-color-hui"><button class="buy bnts bg-color-hui" form-type="submit"
							:class="storeInfo.is_virtual?'virbnt':''">已售罄</button></form>
				</view>
				<view class="bnt acea-row" v-else>
					<form v-if="!storeInfo.is_virtual" @submit="joinCart" class="joinCart bnts"><button
							class="joinCart bnts" form-type="submit">加入购物车</button></form>
					<form @submit="goBuy" class="buy bnts" :class="storeInfo.is_virtual?'virbnt':''"><button
							class="buy bnts" :class="storeInfo.is_virtual?'virbnt':''" form-type="submit">立即购买</button>
					</form>
				</view>
			</view>
			<!-- 	<view v-else>
		...
		methods: {
			...
			getGoodsDetails: function() {
				...
				getProductDetail(that.id)
					.then(res => {
						...
						this.description = this.description.replace(/<img/gi,
			...
			infoScroll: function() {
				...
				for (var i = 0; i < that.navList.length; i++) {
					...
					var idView = '#past' + i;
					if ((!this.replyCount && !that.good_list.length && i == 1) || (this.replyCount && !that.good_list
							.length && i == 2) || (!this.replyCount && that.good_list.length && i == 2)) {
						idView = "#past" + 3;
					}
				...
				uni.getImageInfo({
					...
					success() {
						if (arr2[2] == '') {
							...
							//生成推广海报
							that.$util.PosterCanvas(arr2, that.storeInfo.store_name, that.storeInfo.price, that
								.storeInfo.ot_price,
								function(tempFilePath) {
									that.$set(that, 'posterImage', tempFilePath);
									that.$set(that, 'posterImageStatus', true);
