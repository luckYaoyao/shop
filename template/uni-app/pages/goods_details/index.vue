<template>
	<view class="product-con">
		...
		<view id="home" class="home acea-row row-center-wrapper iconfont icon-xiangzuo" :class="opacity>0.5?'on':''" :style="{ top: homeTop + 'rpx' }" v-if="returnShow" @tap="returns">
					...
					<view class="wrapper">
						<view class="share acea-row row-between row-bottom">
							<view class="money font-color">
								...
								<view class="dis" v-if="storeInfo.vip_price && storeInfo.vip_price > 0 && storeInfo.base">
		...
		<view class="footer acea-row row-between-wrapper">
			...
			<view class="bnt acea-row" v-if="attr.productSelect.stock <= 0">
				<form @submit="joinCart" class="joinCart bnts"><button class="joinCart bnts" form-type="submit">加入购物车</button></form>
				<form class="buy bnts bg-color-hui"><button class="buy bnts bg-color-hui" form-type="submit">已售罄</button></form>
			</view>
		...
		<kefuIcon :ids='parseInt(id)'></kefuIcon>
	</view>
</template>
	...
	methods: {
		...
		getCouponList(type) {
			...
			getCoupons(obj).then(res => {
				...
				if (type === undefined || type === null) {
					let count = [...that.coupon.count],indexs='';
					let index = count.findIndex(item => item);
					let delCount = that.coupon.count,newDelCount = [];
					let countIndex = 0;
